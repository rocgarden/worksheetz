...
begin
  ...
  loop
     ...
     *
     * The outer row is preferred when present. The nested JSON value is
     * accepted as a fallback for legacy drafts.
     */
    v_row_teks :=
      nullif(
        trim(
    ...
    values (
      ...
      /*
       * Question-level TEKS may be the primary passage TEKS or one of its
       * approved supporting TEKS.
       */
      v_question_teks,

      /*
       * Subject and grade remain inherited from the parent passage.
       */
      v_passage.subject,
